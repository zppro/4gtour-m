...
import { Progress, Spinner, Popup, Cell, InfiniteScroll } from 'mint-ui'
...
Vue.http.options.root = process.env.NODE_ENV === 'development' ? 'http://192.168.255.100:3002/me-services' : 'http://sh.okertrip.com/me-services'
...
Vue.http.interceptors.push((request, next) => {
  ...
  oPayload.member = window.proxy.member
  !oPayload.member.member_id && (oPayload.member.member_id = store.state.member.memberId)
  !oPayload.member.member_name && (oPayload.member.member_name = store.state.member.memberName)
